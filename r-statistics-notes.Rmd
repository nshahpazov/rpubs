...
advertisement <- data.frame(tv = x, sales = y)
...
```{r echo=FALSE}
ads_plot_with_ey <- ads_plot +
  geom_line(mapping = aes(x = tv, y = sales - error), color = "#bf8d02")
...
По този начин, получената оценка за $EY$
$$\hat{Y} :=: \hat{f}(x) := \hat{\beta_0}+\hat{\beta_1}x$$

е неизместена оценка.


```{r}
ads_plot_with_ey +
  geom_smooth(method = lm, se = FALSE, color = "#a31898", alpha = 0.5, size = 0.5) +
  ggtitle("Прогноза (виолетово) и Очакване (жълто) за продажбите")
```
...
ggplot(dat, aes(x, y)) +
  ...
  geom_path(data = dens[dens$type == "normal",], aes(x, y, group = section), color = "#336699", alpha = 0.6, lwd = 1.1) +
  theme_bw() +
