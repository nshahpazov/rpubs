...
# 
# anim_save("img/regression-fit2.gif", g)
```
...
advertisement %>%
  ...
  add_row(tv = 144, sales = 0, sample = "Ненаблюдавано х") %>%
...
list(
  ...
  list(df1 = 100, df2 = 1),
  list(df1 = 5, df2 = 10),
  list(df1 = 100, df2 = 100),
  list(df1 = 200, df2 = 200)
) %>%
  imap(
    ~data.frame(
  ...
  ggtitle("F-разпределени случайни величини с различна параметризация") +
  xlab("F") +
  ylab("Емпирична Плътност") +
  xlim(c(0, 4))
```
...
Един начин за намиране на важни и обясняващи $Y$ променливи $Х_1, ..., Х_п$ e първо да пробваме с празен модел
...
$$
T := \frac{\hat{\beta}_0 - 0}{\hat{sd}(\hat{\beta_0})} = \frac{\hat{\beta}_0 - 0}{\hat{\sigma}/\sqrt{n}} \sim \mathcal{T}(n-1)
$$
...
Но какво правим, когато имаме две извадки, всяка с размер n и искаме да направим тест дали идват от популации с еднакви средни?
...
$\hat{\beta}_1$ e оценка за разликата в средните. Ако тестваме дали $\beta_0 = 0$, реално тестваме дали 
има разлика в средните популациите от които произлизат двете извадки.
...
# сметки за наклона
# (x - mean(x)) * (y - mean(y))
# (-0.5 * y1 + 0.5 * y2) = -0.5 * (y2 - y1), можем да преместим 0.5 в знаменателя и да стане 2 там
...
data.frame(y = y, x = x, sample = rep(c("Извадка 1", "Извадка 2"), each = 50)) %>%
  ...
  ggtitle("Две извадки с линеен модел") +
...
Което е същото като да използваме функцията **t.test** за две извадки
...
В горния пример, бихме могли да тестваме, дали променливата Х има ефект върху модела и по друг начин. Ако изпълниме F-тест, той на практика 
би проверил точно това - дали $\beta_1 = 0$ и дали втората извадка статистически значително променя цялостното средно, тоест дали подобрява модела и  драстично се различават двете извадки, защото $\beta_j$ ще представлява разликата между

$$
H_0 := \{ \beta_1  = 0 \iff \bar{Y} = \beta_0 \iff \text{Извадка 2 e същата като Извадка 1 спрямо истинско средно} \} \\
...
lm(y ~ x) %>%
  ...
  summary()
```
Ако разширим модела си с повече от две извадки, изпълнявайки $F$-тест, всъщност ще питаме дали има извадка чиято популация драстично се различава от тази на другите.
...
y1 <- rnorm(n, mean = 10, sd = sigma)
y2 <- rnorm(n, mean = 11, sd = sigma)
y3 <- rnorm(n, mean = 9, sd = sigma)
y4 <- rnorm(n, mean = 10.5, sd = sigma)
y5 <- rnorm(n, mean = 9.5, sd = sigma)

y <- c(y1, y2, y3, y4, y5)
x1 <- rep(c(0, 1, 0, 0, 0), each = n)
x2 <- rep(c(0, 0, 1, 0, 0), each = n)
x3 <- rep(c(0, 0, 0, 1, 0), each = n)
x4 <- rep(c(0, 0, 0, 0, 1), each = n)
x <- rep(1:5, each = n)

# визуализация на данните и теста
data.frame(y = y, x = x, sample = rep(paste0("Извадка ", 1:5), each = n)) %>%
  ggplot(mapping = aes(x = x, y = y, color = sample)) +
  geom_jitter() +
  geom_point(aes(x = 1, y = mean(y1)), colour = "#667350", fill = "#667350", shape = 24, size = 2, alpha = 0.6) +
  geom_point(aes(x = 2, y = mean(y2)), colour = "#667350", fill = "#667350", shape = 24, size = 2, alpha = 0.6) +
  geom_point(aes(x = 3, y = mean(y3)), colour = "#667350", fill = "#667350", shape = 24, size = 2, alpha = 0.6) +
  geom_point(aes(x = 4, y = mean(y4)), colour = "#667350", fill = "#667350", shape = 24, size = 2, alpha = 0.6) +
  geom_point(aes(x = 5, y = mean(y5)), colour = "#667350", fill = "#667350", shape = 24, size = 2, alpha = 0.6) +
  theme_minimal() +
  ylab("Стойности на наблюденията") +
  ggtitle("Пет извадки") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
