...
# Оценка за коефициента бета 1
beta1_estimate <- sum((x - mean(x)) * (y - mean(y))) / sum((x - mean(x))^2)

# Оценка за коефициента бета 0
...
advertisement %>%
  ...
  geom_smooth(method = "lm", fill = NA, lwd = 1, color = "#a8d936") +
  geom_point() +
...
В **R** това може да стане така

```{r}
p <- lm(sales ~ tv, advertisement) %>%
  predict(data.frame(tv = c(300)), se.fit = TRUE, interval = "confidence", level = 0.95)

predict.lm(
  lm(sales ~ tv, advertisement),
  data.frame(tv = c(300)), #, 400, 500)),
  interval = "confidence",
  level = 0.95,
  type = "response"
)
