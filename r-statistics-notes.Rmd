...
advertisement %>%
  ...
  add_row(tv = 144, sales = 266, sample = "Ненаблюдавано х") %>%
...
=\frac{\sigma^2}{n}+\frac{\bar{x}^2\sigma^2}{S_{xx}} + \frac{x_0\sigma^2}{S_{xx}} - 
...
В **R** горният модел се напасва по-следния начин

```{r}
ads_lm_model <- lm(sales ~ tv, data = advertisement)
ads_lm_model
```

По-подробна информация можем да изведем със **summary**

```{r}
summary(ads_lm_model)
```

Ако искаме да изградим прогнози за определени стойности на TV реклама (х)
