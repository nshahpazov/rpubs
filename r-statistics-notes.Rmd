...
Нека наблюдаваме данни идващи от нормално разпределение, примерно от $\mathcal{N}(176, 6^2)$
...
bind_rows(
  ...
  ylab("Наблюдения") +
  theme_minimal() +
  ggtitle("Наблюдения и плътности на две групи") +
  theme_pomological_nobg(base_family = "Pacifico", base_size = 16) +
  scale_fill_pomological() + 
  scale_color_pomological()
```
...
seq(8, 80, by = 5) %>%
  ...
  ggtitle("Хи-квадрат разпределения") +
...
ads_plot <- advertisement %>%
  ...
  geom_point(color = "orange", alpha = 0.6, size = 2) +
...
```{r fitted-model-line-plot, eval=FALSE, include=FALSE}
# advertisement %>%
#   ggplot(mapping = aes(x = tv, y = sales)) +
...
```
```{r creating-regression-fitting, eval=FALSE, include=FALSE}
...
# anim_data <- bind_rows(
#   mutate(
#     advertisement, intercept = 200, slope = 0, state = as.factor(1),
#     fitted_points = 200  + 0 * tv
#   ),
#   mutate(
#     advertisement,
#     intercept = beta0_estimate,
#     slope = beta1_estimate,
#     state = as.factor(2),
#     fitted_points = beta0_estimate + beta1_estimate * tv
#   )
# )
# g <- anim_data %>%
#   ggplot(mapping = aes(x = tv, y = sales)) +
#   geom_point(color = "#f48830", size = 2) +
#   xlab("Бюджет вложен в TV Реклами") +
#   ylab("Печалби") +
#   # theme_pomological(base_family = "Pacifico", base_size = 16) +
#   geom_abline(
#     mapping = aes(intercept = intercept, slope = slope, size  = 0.5, alpha = 0.2),
#     color = "#f48830",
#     size = 0.5,
#     alpha = 0.2
#   ) +
  # geom_segment(
  #   mapping = aes(xend = tv, yend = fitted_points),
  #   color = "##f48830",
  #   alpha = 0.4
  # ) +
  # transition_states(
  ...
  # ggtitle("Напасване на модел") +
  # theme_pomological_nobg(base_family = "Pacifico", base_size = 16) +
  # scale_fill_pomological() + 
  # scale_color_pomological()

# anim_save("img/regression-fit3.gif", g)
```


![](img/regression-fit2.gif)
...
estimates %>%
  ...
  ggtitle("Съвместно плътностно разпределение на оценките на β0 и β1") +
  xlab("Оценка на β1") +
  ylab("Оценка на β0") +
...
Така получаваме [неизместена оценка](https://en.wikipedia.org/wiki/Bias_of_an_estimator) $\hat{\sigma}^2 := \frac{\hat{RSS}}{n-2}$

<!-- с дисперсия $2\sigma^4/(n-2)$. -->
Получената оценка можем да използваме в гореполучените дисперсии на $\hat{\beta_0}$ и $\hat{\beta_1}$ за да изградим оценки за 
техните дисперсии.
...
$$
Z_0 := \frac{\hat{\beta}_0 - \beta_0}{\sqrt{Var(\hat{\beta}_0)}}:= \frac{\hat{\beta}_0 - \beta_0}{\sqrt{\frac{\sigma^2}{n}+\frac{\bar{x}^2\sigma^2}{\sum_{i=1}^n(x_i-\bar{x})^2}}}
\sim \mathcal{N}(0, 1)
$$
...
$$
T_0 := \frac{\hat{\beta_0}-\beta_0}{\sqrt{\hat{Var}(\hat{\beta}_0)}} = \frac{\hat{\beta}_0 - \beta_0}{\sqrt{\frac{\hat{\sigma}^2}{n}+\frac{\bar{x}^2\hat{\sigma}^2}{\sum_{i=1}^n(x_i-\bar{x})^2}}}
\sim \mathcal{T}(n - 2)
$$

то можем да изградим $\gamma$-доверителни интервали, както и да тестваме различни хипотези на базата
на горните статистики.

Аналогични разсъждения са валидни и за $\beta_1$, тъй като

$$
Z_1 := \frac{\hat{\beta}_1-\beta_1}{\sqrt{Var(\hat{\beta_1})}}=
\frac{(\hat{\beta}_1-\beta_1)\sqrt{\sum_{i=1}^n(x_i-\bar{x})^2}}{\sigma}
\sim \mathcal{N}(0, 1)
$$
и

$$
T_1 := \frac{\hat{\beta}_1-\beta_1}{\sqrt{\hat{Var}(\hat{\beta}_1)}} = 
\frac{(\hat{\beta}_1-\beta_1)\sqrt{\sum_{i=1}^n(x_i-\bar{x})^2}}{\hat{\sigma}}
\sim \mathcal{T}(n - 2)
$$
...
Основен въпрос, след като имаме изграден модел, е това да направим прогноза върху ненаблюдавана досега
точка $x_0$. В нашия случай, колко очакваме да спечелим, ако инвестираме 144лв в реклама?

```{r echo=FALSE}
advertisement %>%
  mutate(sample = "Наблюдавани данни") %>%
  add_row(tv = 144, sales = 230, sample = "Ненаблюдавано х") %>%
  ggplot(mapping = aes(x = tv, y = sales, color = sample)) +
  geom_smooth(method = "lm", fill = NA, lwd = 1, color = "gray") +
  geom_point() +
  # theme_pomological_nobg(base_family = "Pacifico", base_size = 16) +
  # scale_fill_pomological() + 
  # scale_color_pomological() +
  ggtitle("Ненаблюдавани досега данни") +
  xlab("Инвестиции в TV реклами") +
  ylab("Продажби") +
  theme_pomological_nobg(base_family = "Pacifico", base_size = 16) +
  scale_fill_pomological() +
  scale_color_pomological()
...
$$
Е[\hat{y}_{x_0} - y_{x_0}] = E[\hat{\beta}_0 + \hat{\beta}_1 x_0 - \beta_0 - \beta_1x_0] =\\
= E[\hat{\beta}_0] + E[\hat{\beta}_1 x_0] - \beta_0 - \beta_1x_0 =\\
= \beta_0 + \beta_1x_0 - \beta_0 - \beta_1x_0 = 0
$$

Дисперсията на тази оценка е сумата от дисперсиите на оценките на коефициентите

$$
Var[\hat{y}_{x_0}] = Var[\hat{\beta}_0 + \hat{\beta}_1 x_0] = \\
= Var[\hat{\beta}_0] + Var[\hat{\beta}_1 x_0] + 2Cov[\hat{\beta}_0, \hat{\beta}_1 x_0] =\\
=\frac{\sigma^2}{n}+\frac{\bar{x}^2\sigma^2}{S_{xx}} + \frac{x_0^2\sigma^2}{S_{xx}} - 
2\frac{x_0\bar{x}\sigma^2}{S_{xx}} = \\
= \sigma^2(\frac{1}{n} + \frac{(\bar{x}-x_0)^2}{S_{xx}})
$$

където, за леснота сме положили $S_{xx} := \sum_{i=1}^n{(x_i-\bar{x})^2}$.

Ако правим оценка за дисперсията на $\hat{y}_{x_0}$, необходимо е в горната формула просто да 
заместим дисперсията $\sigma^2$ с оценката за дисперсията $\hat{\sigma}^2$.

$$
\hat{Var}[\hat{y}_{x_0}] = \hat{\sigma}^2(\frac{1}{n} + \frac{(\bar{x}-x_0)^2}{S_{xx}})
$$

Oсвен точкова оценка за очакваната стойност в $x_0$, можем да изградим и интервална такава посредстом
следната $T_{x_0}$-статистика
...
След като сме отхвърлили хипотезата, че $\beta_1 = 0$, тоест това че $x$ няма ефект върху $y$, бихме могли да попитаме каква е разликата между 
напаснатия модел $\hat{Y}$ и вектора $Y$, тоест
...
Примерно, да разширим горния случай с обясняване на печалбите не само с реклами в телевизия, но и с реклами в 
радио, вестници, тн. В този случай нашия модел за Y e

$$
y_i = \beta_0 + \beta_1 x_{i1} + ... + \beta_p x_{ip} + \epsilon_i
$$

където i варира от 1 до n (брой наблюдения), $\epsilon_i$ са независими наблюдения от нормално разпределена 
случайна грешка $\epsilon \sim \mathcal N(0, \sigma^2)$, a $\beta_j$ e връзката между $Y$ и независимата променлива $Х_j$. Интерпретираме $\beta_j$ като **средния ефект върху $Y$ при една единица промяна в $X_j$, държейки другите променливи фиксирани**.
...
set.seed(1234)
n <- 100
x1 <- runif(n, min = 1, max = 400)
x2 <- runif(n, min = 1, max = 400)
x3 <- rnorm(n) > 0.5

y <- 2 * x1 - x2 + rnorm(n, sd = 120)
...
x1.seq <- seq(min(x1),max(x1),length.out = 50)
x2.seq <- seq(min(x2),max(x2),length.out = 50)
...
p <- plot_ly(x = ~x1.seq, y = ~x2.seq, z = ~z,
  ...
  layout(scene = list(
    ...
    xaxis = list(title = "Х1 = Инвестиции в TV реклами", range = c(0, 400)),
    yaxis = list(title = "X2 = Инвестиции в Интернет реклами", range = c(0, 400)),
    zaxis = list(title = "Y = Печалби", range = pretty(z)[c(1, 8)])))
p
```
...
* Има ли поне една от променливите $X_1, ..., X_p$ която е полезна за напасването на модела?
* Всички независими променливи (предиктори) $X_1,...,X_p$ ли са полезни в това да обяснят $Y$?
* Колко добре е напаснат модела?


#### 1. Има ли връзка между **отговора (зависимата променлива)** $Y$ и независимите променливи (предиктори) $X_1, ..., X_p$?

В простата линейна регресия, за да проверим дали $X$ има ефект върху $Y$, беше необходимо просто да проверим хипотезата дали $\beta_1 = 0$. Когато имаме повече променливи, въпросът се променя в това да проверим хипотезата 
дали $H_0 := \beta_1 = \beta_2 = ... = \beta_p = 0$ срещу алтернативата $H_1 := \exists j : \beta_j \neq 0$.

Нека първо дефинираме няколко помощни метрики

Където
$$
TSS := \rho(Y, \bar{Y})^2 = ||Y - \bar{Y}||_2^2 = \sum_{i=1}^n(y_i-\bar{y})^2\\
RSS := \rho(Y, \hat{Y})^2 = ||Y - \hat{Y}||_2^2 = \sum_{i=1}^n(y_i-\hat{y})^2\\
SSR := \rho(\hat{Y}, \bar{Y})^2 = ||\hat{Y} - \bar{Y}||_2^2 = \sum_{i=1}^n(\hat{y_i}-\bar{y})^2
$$
...
TSS = RSS + SSR
$$
Нека дефинираме следната F-статистика
...
Така, ако нулевата хипотеза, че $\beta_1 = \beta_2 = ... = \beta_p = 0$, би трябвало да имаме $F$-статистика 
близка до единица, защото вариацията обяснена от модела не би трябвало да се различава особено от тази която не е обяснена от модела (модела не носи стойност и $\beta_1 = \beta_2 = ... = \beta_p = 0$).
...
Е[SSR / p] = \sigma^2
$$
което отново показва, че ако нулевата хипотеза е вярна, $F$-статистиката би била близка до 1.

Освен това, $(TSS - RSS)/\sigma^2 \sim \chi^2_{p}$ и $RSS/\sigma^2 \sim \chi^2_{n-p-1}$, откъдето виждаме, че $F$-статистиката е
деление на две нормирани Хи-квадрат разпраделени случайни величини. Полученото разпределение за $F$ се нарича
$\mathcal{F}$-разпределение с параметри $df_1$ и $df_2$ които са степените на свобода на хи-квадрат разпределените случайни величини в знаменател и числител.

По този начин, можем да измерим вероятността да наблюдаваме определена $F$-статистика (или по-крайна) с функцията 
**pf(q, df1, df2)**. Ако съответната вероятност е прекалено малка (примерно по-малка от $\alpha := 0.05$), можем
да отхвърлим нулевата хипотеза $H_0 := \beta_1 = \beta_2 = ... = \beta_p = 0$ и да примем алтернативата, че 
съществува променлива $X_j$ която подобрява модела.


```{r echo=FALSE}
# Същото като rf

simulate_f <- function (n, df1, df2) (rchisq(n, df1) / df1) / (rchisq(n, df2) / df2)

list(
  list(df1 = 1, df2 = 2), list(df1 = 100, df2 = 1), list(df1 = 100, df2 = 100)
) %>%
  imap(
    ~data.frame(
      x = simulate_f(2000, .x$df1, .x$df2),
      distribution = paste0("F(",.x$df1, ", ", .x$df2, ")")
    )
  ) %>%
  bind_rows() %>%
  ggplot(mapping = aes(x = x, fill = distribution)) +
  geom_density(alpha = 0.4) +
  ggtitle("F-разпределени случайни величини") +
  xlab("F") +
  ylab("Емпирична Плътност") +
  theme_pomological_nobg(base_family = "Pacifico", base_size = 16) +
  scale_fill_pomological() + 
...
error <- rnorm(100, sd = 40)

advertisement2 <- data.frame(
  tv = runif(100, 1, 400),
  radio = runif(100, 1, 400),
  newspaper = runif(100, 1, 400),
  facebook = runif(100, 1, 400),
  google = runif(100, 1, 400)
) %>%
  mutate(
    sales = beta0 + beta1 * tv + beta2 * radio + beta3 * newspaper + beta4 * facebook + error
  )
```
...
Y = \beta_0 + \epsilon
$$
и след това да напаснем p на брой линейни модела за всяка променлива $X_p$

$$
Y = \beta_0 + \beta_1 X_j + \epsilon
$$
И да изберем $X_j$ такова, че $R^2$ да увеличи максимално (или RSЕ да се намали максимално). 
Повтаряме процеса, като вече добавяме недобавяна досега променлива към модела $Y = \beta_0 + \beta_1 X_j + \epsilon$, 
тоест
$$
Y = \beta_0 + \beta_1 X_j + \beta_1 X_k  +\epsilon
$$
Продължаваме докато се изпълни някакво зададено от нас терминално условие.

## F-тестове и Анализ на Вариациите

Ако тестваме линейна регресия без независима променлива $X$, тоест $Y = \beta_0 + \epsilon$, забелязваме че 
$$
\hat{\beta}_0 = \bar{y}
$$
а това да тестваме дали $\beta_0 = 0$, се транслира до
$$
T := \frac{\hat{\beta}_0 - 0}{\hat{sd}(\hat{\beta_0})} = \frac{\bar{y} - 0}{\hat{\sigma}/\sqrt{n}} \sim \mathcal{T}(n-1)
$$
...
Или същото с линейна регресия

```{r}
# линейна регресия без променливи х (оценява само средното на у)
lm(y ~ 1) %>%
...
```{r}
# линейна регресия без променливи х (оценява само средното на у с изместване единица)
lm(y - 1 ~ 1) %>%
  # обобщение
  summary() %>%
...
където Y e конкатенация на двете извадки, а $X = \unicode{x1D7D9}_{Y \in \text{извадка 2}}$, 
тоест $x_i = 0$ за $i = 1,..., n$ и $x_i = 1$ за $i = n + 1, ...., 2n$

В горното уравнение на линия, отново $\beta_1$ представлява наклона който имаме. В този смисъл
$$
\beta_1 = \frac{\Delta y}{\Delta x} = \frac{\Delta y}{1} = \Delta y
$$


![](img/slope.png){ height: 100px; width: 100px; }

Така, ако теглиме от първата извадка, оценката за $Y$ става $\hat{Y} = \hat{\beta}_0 = \bar{Y}_{\text{извадка 1}}$, 
а ако теглим от втората извадка $\hat{Y} = \hat{\beta}_0 + \hat{\beta}_11 = \hat{\beta}_0 + \hat{\beta}_1$ и така 
$\hat{\beta}_1$ e оценка за разликата в средните. Ако тестваме дали $\beta_1 = 0$, реално тестваме дали 
има разлика в средните на популациите от които произлизат двете извадки.

Оценката за $\beta_1$ в случая ще бъде

$$
\hat{\beta_1} = \frac{\sum_{i=1}^{2n}(x_i-\bar{x})(y_i-\bar{y})}{\sum_{i=1}^{2n}(x_i-\bar{x})^2} = \\
= \frac{\sum_{i=1}^{n}(0 - 0.5)(y_i-\bar{y}) + \sum_{i=n+1}^{2n}(1 - 0.5)(y_i-\bar{y})}{\sum_{i=1}^{2n}(0.5)^2} = \\
= \frac{0.5(\sum_{i=n+1}^{2n}y_i - \sum_{i=1}^{n}y_i)}{2n(0.5)^2} = \\
\frac{\sum_{i=n+1}^{2n}y_i - \sum_{i=1}^{n}y_i}{n} = \bar{y}_{\text{извадка 1}} - \bar{y}_{\text{извадка 2}}
$$
...
# сметки за наклона
# (x - mean(x)) * (y - mean(y))
# (-0.5 * y1) + (0.5 * y2) = -0.5 * (y2 - y1), можем да преместим 0.5 в знаменател и да стане 2 там
# 2 * sum((x - mean(x))^2) => делим на 50 => имаме осредняване, защото 50 e броя на елементи в двете извадки
# => mean(y2 - y1)

# визуализация на данните и теста
data.frame(y = y, x = x, sample = rep(c("Извадка 1", "Извадка 2"), each = 50)) %>%
  ggplot(mapping = aes(x = x, y = y, color = sample)) +
  geom_jitter() +
  geom_hline(yintercept = mean(y1), linetype = "dashed", color = "#f65c57", size = 0.79, alpha = 0.6) +
  geom_hline(yintercept = mean(y2), linetype = "dashed", color = "#00b4b8", size = 0.79, alpha = 0.6) +
  geom_abline(
    intercept = mean(y1),
    slope = mean(y2 - y1),
    linetype = "dashed",
  ...
  scale_fill_pomological() + 
  scale_color_pomological() +
  ylab("Стойности на наблюденията") +
  ggtitle("Две извадки с линейна регресия") +
...
#### ANOVA (**Ан**ализ на **Ва**риациите)

В горния пример, бихме могли да тестваме по още един начин, дали променливата Х (тоест дали двете извадки се различават 
значително) има ефект върху модела. Ако изпълниме F-тест, той на практика 
би проверил точно това - дали $\beta_1 = 0$, тоест дали втората извадка статистически значително променя цялостното средно (има статистически значима разлика между двете средни).

$$
H_0 := \{ \beta_1  = 0 \iff \bar{Y} = \beta_0 \iff \text{Извадка 2 e същата като Извадка 1
спрямо математическото очакване} \} \\
...
$$
H_0 := \{ \beta_1  = \beta_2 = ... = \beta_p = 0 \iff Y = \beta_0 \iff \text{Всички извадки идват от една и съща популация} \} \\
...
# визуализация на данните и теста
samples_plot <- groups_data %>%
  ggplot(mapping = aes(x = x, y = y, color = sample)) +
  geom_jitter() +
  geom_point(
    aes(x = 1, y = mean(y1)), colour = "#667350",
    fill = "#667350", shape = 24, size = 2, alpha = 0.6
  ) +
  geom_point(
    aes(x = 2, y = mean(y2)), colour = "#667350",
    fill = "#667350", shape = 24, size = 2, alpha = 0.6
  ) +
  geom_point(
    aes(x = 3, y = mean(y3)), colour = "#667350",
    fill = "#667350", shape = 24, size = 2, alpha = 0.6
  ) +
  geom_point(
    aes(x = 4, y = mean(y4)), colour = "#667350",
    fill = "#667350", shape = 24, size = 2, alpha = 0.6
  ) +
  geom_point(
    aes(x = 5, y = mean(y5)), colour = "#667350",
    fill = "#667350", shape = 24, size = 2, alpha = 0.6
  ) +
  theme_pomological_nobg(base_family = "Pacifico", base_size = 16) +
  scale_fill_pomological() + 
...
# the plot plus lines from the mean
samples_plot
```

Ако го разглеждаме от гледна точка на линейна регресия с много променливи (индикатори за съответните групи), оценките за $\beta_1, .., \beta_p$ ще са разликите между средните на групи 2,3,..., р и първата група (визуално това са наклони в отделни линии). В долната графика, пресечната линия е средното на първата група (оценката за $\beta_0$).

```{r echo=FALSE}
samples_plot +
  geom_hline(yintercept = mean(y1), color = "#667350", linetype = "dashed", alpha = 0.4) +
  geom_segment(
    mapping = aes(x = 0.5, y = mean(y1), xend = 1, yend = mean(y1)),
    color = "#667350",
    # size = 0.4,
    alpha = 0.4,
    linetype = "dashed"
  ...
  geom_segment(
    mapping = aes(x = 1.5, y = mean(y1), xend = 2, yend = mean(y2)),
    color = "#667350",
    # size = 0.4,
    alpha = 0.4,
    linetype = "dashed"
  ...
  geom_segment(
    mapping = aes(x = 2.5, y = mean(y1), xend = 3, yend = mean(y3)),
    color = "#667350",
    # size = 0.4,
    alpha = 0.4,
    linetype = "dashed"
  ...
  geom_segment(
    mapping = aes(x = 3.5, y = mean(y1), xend = 4, yend = mean(y4)),
    color = "#667350",
    # size = 0.4,
    alpha = 0.4,
    linetype = "dashed"
  ...
  geom_segment(
    mapping = aes(x = 4.5, y = mean(y1), xend = 5, yend = mean(y5)),
    color = "#667350",
    # size = 0.4,
    alpha = 0.4,
    linetype = "dashed"
...
За да изпълним ANOVA тест който проверява дали някоя от групите значително се различава от останалите
(нейния коефициент значително обяснява модела), пускаме линейна регресия обясняваща y спрямо съответните 
индикаторни променливи $x_1, ..., x_4$.
...
Предположенията на **ANOVA** теста си остават същите като при линейна регресия и Т-тест - данни с нормално
разпределение и еднаква дисперсия между групите.
...
Така получения тест бихме могли да използваме за да проверим дали определен фактор (група) променя 
значително цялата съвкупност.
...
# 
# m2 <- c(0, rep(1, 5), rep(0, 30)) %>% matrix(ncol = 6, byrow = FALSE)
# 
# rownames(m2) <- c("Y", paste0("Група ", 1:5))
# colnames(m2) <- c("Y", paste0("Група ", 1:5))
# 
# chorddiag(m2, groupColors = groupColors, groupnamePadding = 20)
```
### МANOVA (**М**ногопроменлив **Ан**ализ на **Ва**риациите)

Често се налага обаче да проверим дали факторите (групите) имат ефект върху не една променлива, a 
две променливи. Пример за това би бил въпроса дали различни видове лекарства имат значителен ефект 
както върху хемоглобина, така и върху хематокрита.

```{r eval=FALSE, include=FALSE}
# library(chorddiag)
# groupColors <- c("#000000", "#FFDD89", "#957244", "#F26223")
# 
# m3 <- c(0, 0, rep(1, 5), 0, 0, rep(1, 5), rep(0, 35)) %>% matrix(ncol = 7, byrow = FALSE)
# 
# rownames(m3) <- c("A", "___B", paste0("Лекарство ", 1:5))
# colnames(m3) <- c("A", "___B ", paste0("Лекарство ", 1:5))
# 
# chorddiag(m3, groupColors = groupColors, groupnamePadding = 10)
...
Изпълнението на **MANOVA** теста става с функцията **manova**, като 
като аргумент подаваме матрица от двата отклика **manova(cbind(y1, y2) ~ x, data = some_data)**

```{r include=FALSE}
two_response_groups_data <- groups_data %>%
  rename(y1 = y) %>%
  mutate(y2 = c(rnorm(n, 10), rnorm(n, 7), rnorm(n, 19), rnorm(n, 15), rnorm(n, 16))) %>%
  mutate(sample = as.factor(sample), x = as.factor(x))
```
...
За да проверим, колко точно групите афектират двата отклика $y_1$ и $y_2$, можем да използваме 
функцията **summary.aov** за да изпълним поединична **ANOVA**
